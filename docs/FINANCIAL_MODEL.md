...
## Member Monthly View and sharing controls Web implementation notes (`apps/web/src/app/app`)

- `/app` now branches by the caller's active membership role. The Owner branch is unchanged from prior sessions. A Member is instead served `MemberMonthlyViewPanel` (`apps/web/src/app/app/member-view.tsx`), fed by `loadMemberMonthlyView(...)` (`apps/web/src/app/app/member-data.ts`) — a read-only loader that never queries `fixed_commitment_templates`, `period_fixed_commitments`, or `get_owner_period_planning_summary(...)`, and never reads `income_sources`/`period_income_items` directly. It only reads `budget_periods.spending_budget`, the caller's own RLS-visible `period_section_budgets`/`budget_sections`/`transactions` rows, and calls `get_member_visible_total_income(...)` for the optional income total. A Draft period renders a minimal calm state (no financial figures); a Closed period renders the same read-only data as Open but never offers an expense action.
- The "الأقسام المشتركة" shared summary shown to a Member is computed by summing only the section rows RLS already returned to that specific caller (`allocated`/`spent`/`remaining`); it is never combined with `spending_budget` or any other total that could let a hidden `owner_only` section's allocation be inferred by subtraction. No new database RPC was added for this — the existing per-row RLS grant on `period_section_budgets`/`transactions` is sufficient given the small MVP dataset shape.
- The Owner's existing `/app/plan` Sections step (`sections-step.tsx`) gained a per-section sharing control exposing exactly three modes over the existing `budget_sections.visibility_scope`/`member_access` columns: `خاص بيا` (`owner_only`), `مشترك — مشاهدة` (`household`/`view`), `مشترك — مساهمة` (`household`/`contribute`). `custom` is never offered by this control and is never silently rewritten; if a section is already `custom`, the control renders a plain non-destructive note and only replaces it if the Owner explicitly picks one of the three modes. Broadening from a scope a Member cannot currently see (`owner_only` or an existing `custom` grant) into either `household` mode shows an inline confirmation first, because MVP's live/current-authorization model (D-031) means that change can also expose the section's previously recorded transactions; narrowing access, and switching between the two `household` submodes, applies immediately with no confirmation.
- The Income step (`income-step.tsx`) gained a compact "المشاركة" panel with a single toggle for `households.share_total_income_with_members`, written through the same `households` `update` RLS policy the Owner already has (`setTotalIncomeSharing(...)` in `plan/actions.ts`) — no new RPC. Both new actions (`setTotalIncomeSharing`, `updateSectionSharing`) follow the same `"use server"` + friendly-error-mapping pattern as every other action in that file and call only `revalidatePath("/app/plan")`/`revalidatePath("/app")`, never a service-role client.

## Owner fixed-commitment operations Web implementation notes (`apps/web/src/app/app`)

- The Owner's `/app` landing gained a compact "الالتزامات الثابتة" execution section (`owner-commitments.tsx`/`owner-commitments-data.ts`), separate from the `/app/plan` Commitments step, which remains the planning surface (amount edits, adding commitments, "إيقاف التكرار"). This new section only lets the Owner act on the *current* Budget Period's `period_fixed_commitments` rows: mark `pending` → `paid` (`mark_period_fixed_commitment_paid(...)`, always omitting `p_actual_amount` so the RPC's own default — the planned amount — applies; see "Owner decision" below) and toggle `pending` ↔ `skipped` (`set_period_fixed_commitment_skipped(...)`, which already supports the reverse direction via `p_skip = false`). No new RPC, migration, or table access was added; both operations already existed.
- **Known backend permissiveness, not changed this session:** `mark_period_fixed_commitment_paid(...)` and `set_period_fixed_commitment_skipped(...)` both authorize via `is_period_open_for_writes(...)`, which returns true for `draft` **and** `open` (the same helper `set_period_fixed_commitment_planned_amount(...)` uses for planning edits) — verified empirically against the live stack. This means the database itself does not reject a Paid/Skip call during Draft, only during Closed. The Web UI is intentionally *more conservative* than the backend requires: `OwnerCommitmentsSection` renders zero action buttons (no "تم الدفع", no "تخطي هذا الشهر") whenever the period is `draft`, exposing them only when `open`, matching the approved MVP lifecycle (Draft = planning only). This is a deliberate, disclosed frontend restriction, not a claim that the backend enforces the same boundary — a future session should decide whether to tighten `mark_period_fixed_commitment_paid`/`set_period_fixed_commitment_skipped` to require `open` specifically (mirroring `record_expense`'s stricter check, D-030) if Draft-time execution should be prevented authoritatively rather than just hidden.
- **Owner decision deferred, not silently chosen:** `mark_period_fixed_commitment_paid(p_period_fixed_commitment_id, p_actual_amount default null)` already accepts an actual-amount override distinct from the planned amount, but no product decision establishes whether the Owner should be able to settle a commitment for a different amount than planned. This session's "تم الدفع" always omits `p_actual_amount`, so the recorded actual outflow equals the planned amount — a simple settle action, not an adjustable/partial-payment flow. Whether to expose an editable amount at settlement time remains an open product question for a future session.
- Skip/unskip already existed in `/app/plan`'s Commitments step (usable in both Draft and Open, unchanged by this session) and continues to work there; the new `/app` execution surface reuses the exact same `setCommitmentSkipped` server action (now also revalidating `/app`) so there is one source of truth for that mutation across both surfaces.
- Read data (`loadOwnerCommitments(...)`) never queries anything beyond `period_fixed_commitments` for the current period; it relies entirely on that table's existing Owner-only RLS grant, so a Member hitting the same code path receives zero rows rather than an error. Bucketed summary totals (planned/paid/pending/skipped) are plain sums of the exact rows displayed — not a re-derivation of `get_owner_period_planning_summary(...)`'s deeper income/budget arithmetic, which this surface does not touch.
- A commitment's "فات موعده" (overdue) label is a pure display computation (`due_date` compared against "today" in the Household's timezone) for a still-`pending` row; it introduces no new business semantics, no notification, and no severity/penalty language.
