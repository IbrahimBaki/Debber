...
- **Backend permissiveness closed (was previously disclosed, now fixed):** `mark_period_fixed_commitment_paid(...)` and `set_period_fixed_commitment_skipped(...)` (both directions: Pending→Skipped and Skipped→Pending) now authoritatively require the parent Budget Period's `status` to be exactly `open`, raising `period_not_open` (`P0001`) otherwise — mirroring `record_expense`'s stricter check (D-030) rather than reusing `is_period_open_for_writes(...)` (which still returns true for `draft` and remains correct for the planning RPCs: `set_period_fixed_commitment_planned_amount(...)`, section/budget/income edits). Draft is planning-only and Closed is immutable for these two operations specifically; nothing else changed. As a direct consequence, `/app/plan`'s Commitments step no longer renders "تخطي هذا الشهر"/"إلغاء التخطي" while the period is Draft (`CommitmentsStep`/`CommitmentRow` gained a `canOperate` prop, `mode === "open"`, threaded from `PlanClient`) — this is the one small, mechanical Web adjustment this hardening required; the Owner's `/app` "الالتزامات الثابتة" section (added in a prior session) already restricted Paid/Skip to `open` and needed no change.
